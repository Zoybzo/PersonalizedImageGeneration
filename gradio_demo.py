...
def text2image(image_prompt, type, user_info):
    if type == "id":
        ...
        params = {"api_key": "123", "user_id": "123", "prompt": image_prompt}
        response = requests.post(url, data=params)
    else:
        url = base_url + "generateByList"
        params = {"api_key": "123", "id_list": ["123", "123"], "prompt": image_prompt}
        response = requests.post(url, data=params)
...
t2i_demo = gr.Interface(
    ...
    inputs=[
        gr.Textbox(
            placeholder="Enter prompt for image generation", label="Image Prompt"
        ),
